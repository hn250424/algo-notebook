...
int solution(int h1, int m1, int s1, int h2, int m2, int s2) {
    ...
    size_t c_m = m1 * 60 + s1;
    size_t c_h;
    if (h1 >= 12) c_h = (h1 - 12) * 3600 + m1 * 60 + s1;
    else c_h =  h1 * 3600 + m1 * 60 + s1;

    int answer = 0;

    if (c_s * 60 == c_m) answer++;
    if (c_s * 720 == c_h && c_m * 12 != c_h) answer++;

    size_t time = (h2 * 3600 + m2 * 60 + s2) - (h1 * 3600 + m1 * 60 + s1);
    for (int i = time; i > 0; i--) {
        c_s++;
        c_m++;
        c_h++;

        if (c_s * 60 >= c_m && c_m - 1 > (c_s - 1) * 60) answer++;
        if (c_s * 720 >= c_h && c_h - 1 > (c_s - 1) * 720 && c_m * 12 != c_h) answer++;
...
int main() {
    ...
    int given_a[7] = { 2, 1, 0, 1, 1, 2, 2852 };

    for (size_t i = 0; i < 7; i++) {
        int answer = solution(given_q[i][0], given_q[i][1], given_q[i][2], given_q[i][3], given_q[i][4], given_q[i][5]);
        if (answer == given_a[i]) printf("OK.\n");
